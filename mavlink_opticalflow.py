...
CLASSIC = False
MAGIC_CONSTANTS = False
COEFFS = {
    "x": -35 if MAGIC_CONSTANTS else 1,
    "y": 53 if MAGIC_CONSTANTS else 1
}
PIXFORMAT = sensor.RGB565 if CLASSIC else sensor.GRAYSCALE
...
extra_fb = sensor.alloc_extra_fb(sensor.width(), sensor.height(), PIXFORMAT)
extra_fb.replace(sensor.snapshot())

def is_flowhold() -> bool:
    """True if there's no commands to move, False otherwise"""
    pass
...
while True:
    ...
    sub_pixel_x = int(displacement.x_translation() * COEFFS["x"])
    sub_pixel_y = int(displacement.y_translation() * COEFFS["x"])
